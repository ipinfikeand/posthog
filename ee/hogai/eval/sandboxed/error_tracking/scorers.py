...
All scorers walk ``output["messages"]`` and use
``ee.hogai.eval.sandboxed.scorers.enumerate_tool_calls`` so they work
identically in ``mcp_mode=tools`` (per-tool MCP) and ``mcp_mode=cli``
(single-``exec`` wrapper).
"""
...
from ee.hogai.eval.sandboxed.scorers import enumerate_tool_calls, extract_user_prompt
...
def _last_input_for(messages: list[dict[str, Any]] | None, tool_name: str) -> dict[str, Any] | None:
    """Return the input dict of the most recent successful call to ``tool_name``.

    Uses ``enumerate_tool_calls`` so single-exec ``call <tool>`` invocations
    are unwrapped automatically. Returns ``None`` when the tool was never
    called successfully — scorers should short-circuit with ``score=None``
    in that case rather than counting it as a wrong answer.
    """
    if not messages:
        return None
    last: dict[str, Any] | None = None
    for _pos, name, tool_use in enumerate_tool_calls(messages):
        if name != tool_name:
            continue
...
def extract_last_query_issues_list_input(output: dict[str, Any] | None) -> dict[str, Any] | None:
    if not isinstance(output, dict):
        return None
    return _last_input_for(output.get("messages"), QUERY_ISSUES_LIST_TOOL)


def extract_last_query_issue_input(output: dict[str, Any] | None) -> dict[str, Any] | None:
    if not isinstance(output, dict):
        return None
    return _last_input_for(output.get("messages"), QUERY_ISSUE_TOOL)


def extract_last_query_issue_events_input(output: dict[str, Any] | None) -> dict[str, Any] | None:
    if not isinstance(output, dict):
        return None
    return _last_input_for(output.get("messages"), QUERY_ISSUE_EVENTS_TOOL)
...
class _AlignmentClassifier(LLMClassifier):
    ...
    def _prepare(self, output: dict[str, Any] | None, expected: dict[str, Any] | None) -> dict[str, Any] | Score:
        ...
        return {
            "output": {"actual": actual, "prompt": prompt},
...
class IssueIdMatchesTarget(Scorer):
    ...
    def _evaluate(self, output: dict | None, expected: dict | None) -> Score:
        ...
        for _pos, name, tool_use in enumerate_tool_calls(messages or []):
            if name not in {QUERY_ISSUE_TOOL, QUERY_ISSUE_EVENTS_TOOL}:
                continue
            tool_input = tool_use.get("input")
            issue_id = tool_input.get("issueId") if isinstance(tool_input, dict) else None
            if isinstance(issue_id, str):
                observed.append((name, issue_id))
...
class IssueDrilldownOrder(Scorer):
    ...
    def _evaluate(self, output: dict | None, expected: dict | None) -> Score:
        ...
        list_pos = self._first_pos(messages, QUERY_ISSUES_LIST_TOOL)
        issue_pos = self._first_pos(messages, QUERY_ISSUE_TOOL)
        events_pos = self._first_pos(messages, QUERY_ISSUE_EVENTS_TOOL)
        recordings_pos = self._first_pos(messages, SESSION_RECORDINGS_LIST_TOOL)
        ...
        if requires_recordings:
            ...
            if recordings_pos < min_pred:
                metadata["reason"] = (
                    f"{SESSION_RECORDINGS_LIST_TOOL} ran before its prerequisite drill-down"
                )
                return Score(name=self._name(), score=0.0, metadata=metadata)
    ...
    @staticmethod
    def _first_pos(messages: list[dict[str, Any]], tool_name: str) -> int | None:
        """Earliest position of a successful call to ``tool_name``."""
        for pos, name, _tool_use in enumerate_tool_calls(messages):
            if name == tool_name:
                return pos
        return None
