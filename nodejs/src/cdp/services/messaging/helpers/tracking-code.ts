...
export const parseEmailTrackingCode = (
    ...
    const sepIdx = encodedTrackingCode.indexOf('.')
    if (sepIdx !== -1) {
        const signature = encodedTrackingCode.slice(sepIdx + 1)
        payloadB64 = encodedTrackingCode.slice(0, sepIdx)
    ...
    try {
        ...
        // newer codes append parentRunId as a 5th segment.
