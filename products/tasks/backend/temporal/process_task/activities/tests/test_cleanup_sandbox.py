...
from products.tasks.backend.temporal.process_task.activities.cleanup_sandbox import (
    ...
    AGENT_SERVER_GRACEFUL_SHUTDOWN_TIMEOUT_SECONDS,
    CleanupSandboxInput,
    cleanup_sandbox,
)
...
@pytest.mark.django_db
def test_cleanup_sandbox_requests_agent_server_shutdown_before_destroy_when_completing_stream(
    activity_environment, mocker
):
    sandbox = mocker.Mock(id="sandbox-123")
    sandbox.execute.return_value = SimpleNamespace(exit_code=0, stderr="")
    get_by_id = mocker.patch.object(Sandbox, "get_by_id", return_value=sandbox)
...
@pytest.mark.django_db
def test_cleanup_sandbox_destroys_when_agent_server_shutdown_fails(activity_environment, mocker):
    sandbox = mocker.Mock(id="sandbox-123")
    sandbox.execute.side_effect = RuntimeError("shutdown failed")
...
@pytest.mark.django_db
def test_cleanup_sandbox_destroys_when_agent_server_shutdown_returns_nonzero(activity_environment, mocker):
    sandbox = mocker.Mock(id="sandbox-123")
    sandbox.execute.return_value = SimpleNamespace(exit_code=1, stderr="still running")
    mocker.patch.object(Sandbox, "get_by_id", return_value=sandbox)
...
@pytest.mark.django_db
def test_cleanup_sandbox_does_not_complete_stream_when_producer_might_still_be_running(activity_environment, mocker):
    sandbox = mocker.Mock(id="sandbox-123")
    sandbox.execute.side_effect = RuntimeError("shutdown failed")
