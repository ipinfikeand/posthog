...
async def test_start_agent_server_uses_captured_sandbox_event_ingest_flag(mocker) -> None:
    ...
    sandbox.start_agent_server.side_effect = lambda *args, **kwargs: order.append("start_agent_server")
    mocker.patch(
    ...
    update_state_atomic.assert_called_once_with(
        "run-id",
        updates={"sandbox_event_ingest_producer_started": True},
    )
    sandbox.start_agent_server.assert_called_once()
    assert order == ["producer_started", "start_agent_server"]
