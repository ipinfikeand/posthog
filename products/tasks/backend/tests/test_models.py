...
class TestTaskRun(TestCase):
    ...
    @parameterized.expand(
        [
            ("in_progress", TaskRun.Status.IN_PROGRESS, {}, False),
            ("completed_without_sandbox_ingest", TaskRun.Status.COMPLETED, {}, True),
            (
                "completed_before_sandbox_ingest_producer_starts",
                TaskRun.Status.COMPLETED,
                {"sandbox_event_ingest_enabled": True},
                True,
            ),
            (
                "completed_after_sandbox_ingest_producer_starts",
                TaskRun.Status.COMPLETED,
                {"sandbox_event_ingest_enabled": True, "sandbox_event_ingest_producer_started": True},
                False,
            ),
            (
                "completed_after_sandbox_ingest_producer_confirmed",
                TaskRun.Status.COMPLETED,
                {
                    "sandbox_event_ingest_enabled": True,
                    "sandbox_event_ingest_producer_started": True,
                    "sandbox_event_ingest_producer_confirmed": True,
                },
                False,
            ),
        ]
    )
    @patch("products.tasks.backend.models.publish_task_run_stream_complete")
    @patch("products.tasks.backend.models.publish_task_run_stream_event")
    def test_publish_stream_state_event_completion(
        self, _name, status, state, should_complete_stream, mock_publish_stream_event, mock_publish_stream_complete
    ):
        run = TaskRun.objects.create(
            task=self.task,
            team=self.team,
        ...
        mock_publish_stream_event.assert_called_once()
        if should_complete_stream:
            mock_publish_stream_complete.assert_called_once_with(str(run.id))
        else:
            mock_publish_stream_complete.assert_not_called()
