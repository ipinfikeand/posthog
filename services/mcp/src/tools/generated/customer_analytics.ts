...
const AccountsCreateSchema = AccountsCreateBody.extend({
    properties: AccountsCreateBody.shape['properties'].describe(
        'Typed account properties. `group_type_index` selects which group type the account is attached to and must match the team config. `group_keys` lists the group keys (string identifiers) that belong to this account. `csm`, `account_executive`, `account_owner` are role assignments — each takes `{id, email}` of an existing user.'
    ),
})
...
const AccountsPartialUpdateSchema = AccountsPartialUpdateParams.omit({ project_id: true })
    ...
    .extend({
        properties: AccountsPartialUpdateBody.shape['properties'].describe(
            'Typed account properties. Pass only the fields you want to change — the server replaces the `properties` object as a whole, so include any existing values you want to preserve. `group_keys` must reference groups of the configured `group_type_index`.'
        ),
    })
