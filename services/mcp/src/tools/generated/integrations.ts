...
import { IntegrationsChannelsRetrieveParams, IntegrationsDestroyParams, IntegrationsListQueryParams, IntegrationsRetrieveParams } from '@/generated/integrations/api'
...
const integrationsList = (): ToolBase<typeof IntegrationsListSchema, WithPostHogUrl<Schemas.PaginatedIntegrationConfigList>> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof IntegrationsListSchema>) => {
        ...
        const filtered = { ...result, results: (result.results ?? []).map((item: any) => pickResponseFields(item, ['id', 'kind', 'display_name', 'created_at', 'created_by'])) } as typeof result
