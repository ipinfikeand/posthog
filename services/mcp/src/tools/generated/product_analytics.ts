...
import { InsightsActivityRetrieveParams, InsightsActivityRetrieveQueryParams, InsightsAllActivityRetrieveQueryParams, InsightsCreateBody, InsightsDestroyParams, InsightsListQueryParams, InsightsPartialUpdateBody, InsightsPartialUpdateParams, InsightsRetrieveParams, InsightsRetrieveQueryParams, InsightsTrendingRetrieveQueryParams } from '@/generated/product_analytics/api'

const AssistantInsightVizNode = z.object({
    kind: z.literal("InsightVizNode").default("InsightVizNode"),
    source: z.record(z.string(), z.unknown()).describe('Product analtycs query objects like TrendsQuery, FunnelsQuery, RetentionQuery, PathsQuery, StickinessQuery, LifecycleQuery'),
})
...
const AssistantDataVisualizationAxisDisplaySettings = z.object({
    yAxisPosition: z.enum(["left", "right"]).describe('Which Y axis this numeric series should use. Use `right` for a secondary Y axis.').optional(),
})

const AssistantDataVisualizationAxisSettings = z.object({
    display: AssistantDataVisualizationAxisDisplaySettings.describe('Display settings for a plotted Y series.').optional(),
})
...
const AssistantDataVisualizationAxis = z.object({
    ...
    settings: AssistantDataVisualizationAxisSettings.describe('Optional series settings. Only applies to Y-axis series.').optional(),
})

const AssistantDataVisualizationChartSettings = z.object({
    goalLines: z.array(AssistantDataVisualizationGoalLine).describe('Horizontal goal lines drawn across the chart.').optional(),
    leftYAxisSettings: AssistantDataVisualizationYAxisSettings.describe('Settings for the left Y axis.').optional(),
    rightYAxisSettings: AssistantDataVisualizationYAxisSettings.describe('Settings for the right Y axis. Only applies when a Y series uses `settings.display.yAxisPosition: "right"`.').optional(),
    seriesBreakdownColumn: z.string().nullable().describe('Column that splits a single Y series into multiple colored series — e.g. breaking down a line chart by `country`. Set to `null` or omit to disable.').optional(),
    ...
    stackBars100: z.coerce.boolean().describe('Stack bars to 100% of the total. Only meaningful with `ActionsStackedBar`.').optional(),
    xAxis: AssistantDataVisualizationAxis.describe('Column used as the X axis. Typically a time bucket or categorical column.').optional(),
    xAxisLabel: z.string().describe('Label rendered under the X axis.').optional(),
    yAxis: z.array(AssistantDataVisualizationAxis).describe('One or more numeric columns plotted as Y series.').optional(),
})

const AssistantDataVisualizationDisplayType = z.enum(["ActionsTable", "BoldNumber", "ActionsLineGraph", "ActionsBar", "ActionsStackedBar", "ActionsAreaGraph", "TwoDimensionalHeatmap"])

const AssistantDataVisualizationTableSettings = z.object({
    columns: z.array(AssistantDataVisualizationAxis).describe('Columns to display and their order. Omit to show every column returned by the query.').optional(),
...
const AssistantDataVisualizationNode = z.object({
    chartSettings: AssistantDataVisualizationChartSettings.describe('Chart configuration. Ignored when `display` is `ActionsTable` or `BoldNumber`.').optional(),
    display: AssistantDataVisualizationDisplayType.describe('Visualization type. Defaults to `ActionsTable` when omitted.\n\nGuidance:\n- Single-value result (one numeric column, one row) → `BoldNumber`.\n- Time series → `ActionsLineGraph` or `ActionsAreaGraph`.\n- Categorical comparison → `ActionsBar` or `ActionsStackedBar`.\n- Two-dimensional aggregation → `TwoDimensionalHeatmap`.\n- Otherwise → `ActionsTable`.').optional(),
    ...
    tableSettings: AssistantDataVisualizationTableSettings.describe('Table configuration. Only applies when `display` is `ActionsTable` or omitted.').optional(),
})

const InsightQuery = z.union([AssistantInsightVizNode, AssistantDataVisualizationNode])

const InsightCreateSchema = (InsightsCreateBody.omit({ 'derived_name': true, 'order': true, 'deleted': true, '_create_in_folder': true })).extend({ query: InsightQuery, dashboards: InsightsCreateBody.shape['dashboards'].describe('Dashboard IDs this insight should belong to. This is a full replacement — always include all existing dashboard IDs when adding a new one.') })
...
const insightCreate = (): ToolBase<typeof InsightCreateSchema, WithPostHogUrl<Schemas.Insight>> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof InsightCreateSchema>) => {
        ...
        if (params.dashboards !== undefined) {body["dashboards"] = params.dashboards}
        ...
        const filtered = omitResponseFields(result, ['result', 'hasMore', 'columns', 'is_cached', 'query_status', 'hogql', 'types']) as typeof result
...
const InsightUpdateSchema = (InsightsPartialUpdateParams.omit({ project_id: true }).extend(InsightsPartialUpdateBody.omit({ 'derived_name': true, 'order': true, 'deleted': true, '_create_in_folder': true }).shape)).extend({ query: InsightQuery.optional(), dashboards: InsightsPartialUpdateBody.shape['dashboards'].describe('Dashboard IDs this insight should belong to. This is a full replacement — always include all existing dashboard IDs when adding a new one.') })
...
const InsightsActivityRetrieveSchema = InsightsActivityRetrieveParams.omit({ project_id: true }).extend(InsightsActivityRetrieveQueryParams.omit({ 'format': true }).shape)

const insightsActivityRetrieve = (): ToolBase<typeof InsightsActivityRetrieveSchema, WithPostHogUrl<Schemas.ActivityLogPaginatedResponse>> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof InsightsActivityRetrieveSchema>) => {
        ...
        const filtered = { ...result, results: (result.results ?? []).map((item: any) => omitResponseFields(item, ['detail.changes.*.before', 'detail.changes.*.after', 'detail.trigger', 'detail.type'])) } as typeof result
...
const insightsAllActivityRetrieve = (): ToolBase<typeof InsightsAllActivityRetrieveSchema, WithPostHogUrl<Schemas.ActivityLogPaginatedResponse>> => ({
...
const InsightsListSchema = (InsightsListQueryParams.omit({ 'format': true, 'basic': true, 'refresh': true })).extend({ limit: z.preprocess(castStringToInt, InsightsListQueryParams.shape['limit']).optional(), offset: z.preprocess(castStringToInt, InsightsListQueryParams.shape['offset']).optional() })
...
const insightsList = (): ToolBase<typeof InsightsListSchema, WithPostHogUrl<Schemas.PaginatedInsightList>> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof InsightsListSchema>) => {
        ...
        const filtered = { ...result, results: (result.results ?? []).map((item: any) => pickResponseFields(item, ['id', 'short_id', 'name', 'derived_name', 'description', 'tags', 'favorited', 'dashboards', 'created_at', 'created_by', 'last_modified_at', 'last_modified_by', 'last_viewed_at', 'alerts'])) } as typeof result
        return await withPostHogUrl(context, {
            ...filtered,
            results: await Promise.all((filtered.results ?? []).map((item) => withPostHogUrl(context, item, `/insights/${item.short_id}`))),
        }, '/insights')
    },
})
...
const insightsTrendingRetrieve = (): ToolBase<typeof InsightsTrendingRetrieveSchema, WithPostHogUrl<Schemas.PaginatedTrendingInsightList>> => ({
    ...
    handler: async (context: Context, params: z.infer<typeof InsightsTrendingRetrieveSchema>) => {
        ...
        const filtered = { ...result, results: (result.results ?? []).map((item: any) => pickResponseFields(item, ['id', 'short_id', 'name', 'derived_name', 'description', 'tags', 'favorited', 'dashboards', 'created_at', 'created_by', 'last_modified_at', 'last_modified_by', 'last_viewed_at', 'view_count', 'viewers'])) } as typeof result
        return await withPostHogUrl(context, {
            ...filtered,
            results: await Promise.all((filtered.results ?? []).map((item) => withPostHogUrl(context, item, `/insights/${item.short_id}`))),
        }, '/insights')
