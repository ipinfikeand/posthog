...
const SdkDoctorGetSchema = (SdkDoctorReportRetrieveQueryParams).extend({ force_refresh: SdkDoctorReportRetrieveQueryParams.shape['force_refresh'].describe('Set to true to bypass the Redis cache and re-query ClickHouse for SDK usage. Use sparingly — data is refreshed every 12 hours by a background job, so the cached answer is usually fine.') })
